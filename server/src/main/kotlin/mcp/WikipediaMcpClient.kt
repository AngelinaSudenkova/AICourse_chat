...
import models.WikipediaSnippet
...
/**
 * MCP client for Wikipedia operations.
 */
class WikipediaMcpClient(
    ...
    private suspend fun ensureInitialized() {
        ...
        try {
            ...
            throw McpException("Failed to start Wikipedia MCP server: ${e.message}", e)
        }
    }
    
    suspend fun getSummary(topic: String): WikipediaSnippet {
        ensureInitialized()
        ...
        if (parsedText is JsonObject && parsedText.containsKey("error")) {
            ...
            throw McpException("Wikipedia API error: $errorMsg")
        }
        
        return json.decodeFromString(WikipediaSnippet.serializer(), text)
    }
